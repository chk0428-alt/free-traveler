# TASK-COMP-SCR002-STORY — 자기소개·철학 2~4문단
...
SCR-002(`/about`) 내부에 들어가는 '자기소개·철학 2~4문단' 컴포넌트를 구현하는 Task다. 이 컴포넌트는 해당 Screen의 Page Owner Task가 조립할 때 사용되며, 독립적으로 라우팅되지 않는다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT**로 분류됨(대상 Requirement: REQ-FUNC-058). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.
...
- [Functional] 확정 소개문·철학·편집 원칙 줄임 없이 표시가 의도대로 동작하는지 확인한다.
- [Visual] 좌우 분할(이미지+텍스트) 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
