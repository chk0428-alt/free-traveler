# TASK-API-MATE-BLOCK — 사용자 차단/해제 Server Action

**Category:** API  
**Implementation Status:** IMPLEMENT  
**Seq:** 19  
**Task ID:** API-MATE-BLOCK

---

## Context

'사용자 차단/해제 Server Action'를 처리하는 Server Action/Route Handler를 구현하는 Task다. Supabase `user_block` 테이블에 대한 쓰기/상태 전이를 담당하며, DB-ACCESS 계층을 통해서만 데이터에 접근한다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT**로 분류됨(대상 Requirement: REQ-FUNC-040). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.
...
- Route: `/mates`, `/account`
- Page Entry: `src/app/mates/actions/block.ts`
...
- [Functional] 차단 후 상호 글/프로필/요청 미노출가 의도대로 동작하는지 확인한다.
- [Functional] 차단 해제 가능가 의도대로 동작하는지 확인한다.
- [Visual] 해당 없음(N/A) 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
- [Security/Privacy] 차단 관계는 본인만 열람(RLS) 조건이 위반되지 않는지 확인한다.
