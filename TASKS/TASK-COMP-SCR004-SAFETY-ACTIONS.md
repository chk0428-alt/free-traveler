# TASK-COMP-SCR004-SAFETY-ACTIONS — 신고·차단 액션 + 신청방법 3단계 + 안전 CTA Banner

**Category:** COMPONENT  
**Implementation Status:** IMPLEMENT  
**Seq:** 44  
**Task ID:** COMP-SCR004-SAFETY-ACTIONS

---

## Context

SCR-004(`/mates`) 내부에 들어가는 '신고·차단 액션 + 신청방법 3단계 + 안전 CTA Banner' 컴포넌트를 구현하는 Task다. 이 컴포넌트는 해당 Screen의 Page Owner Task가 조립할 때 사용되며, 독립적으로 라우팅되지 않는다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT**로 분류됨(대상 Requirement: REQ-FUNC-039, REQ-FUNC-040). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.
...
- API-MATE-BLOCK — 이 Task 시작 전 완료되어 있어야 한다.

## Expected Files

- `src/components/scr004/SafetyActions.tsx`(신규)

**이 목록 밖의 파일은 생성·수정하지 않는다.** 다른 파일 변경이 필요하다고 판단되면 이 Task를 완료 처리하지 말고 범위를 재확인한다.

## Functional AC

- 신고(사유코드+설명) 3초 이내 접수 ID 표시, 차단 시 상호 노출 제한, 신청방법 3단계 안내, `/travel-tools` CTA
...
- 신고자/피신고자 상세는 Admin만 노출

## Test Cases

- [Functional] 신고(사유코드+설명) 3초 이내 접수 ID 표시가 의도대로 동작하는지 확인한다.
- [Functional] 차단 시 상호 노출 제한가 의도대로 동작하는지 확인한다.
- [Functional] 신청방법 3단계 안내가 의도대로 동작하는지 확인한다.
- [Functional] `/travel-tools` CTA가 의도대로 동작하는지 확인한다.
- [Visual] CTA Banner는 코랄 배경 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
- [Security/Privacy] 신고자/피신고자 상세는 Admin만 노출 조건이 위반되지 않는지 확인한다.
