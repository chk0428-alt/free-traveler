# TASK-COMP-SCR003-TIPS — 찾기 Tip 3개 섹션

**Category:** COMPONENT  
**Implementation Status:** IMPLEMENT  
**Seq:** 37  
**Task ID:** COMP-SCR003-TIPS

---

## Context

SCR-003(`/travel-tools`) 내부에 들어가는 '찾기 Tip 3개 섹션' 컴포넌트를 구현하는 Task다. 이 컴포넌트는 해당 Screen의 Page Owner Task가 조립할 때 사용되며, 독립적으로 라우팅되지 않는다.
...
- `src/components/scr003/Tips.tsx`(신규)

**이 목록 밖의 파일은 생성·수정하지 않는다.** 다른 파일 변경이 필요하다고 판단되면 이 Task를 완료 처리하지 말고 범위를 재확인한다.

## Functional AC

- "입력값 비전달", "가격/재고는 외부에서 확인", "탭 이동해도 입력값 유지" 3개 카드

## Visual AC

- 3단계 안내 카드 패턴(D-001 16절)
...
- [Functional] "입력값 비전달"가 의도대로 동작하는지 확인한다.
- [Functional] "가격/재고는 외부에서 확인"가 의도대로 동작하는지 확인한다.
- [Functional] "탭 이동해도 입력값 유지" 3개 카드가 의도대로 동작하는지 확인한다.
- [Visual] 3단계 안내 카드 패턴(D-001 16절) 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
