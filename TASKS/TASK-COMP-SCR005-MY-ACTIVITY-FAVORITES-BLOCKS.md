# TASK-COMP-SCR005-MY-ACTIVITY-FAVORITES-BLOCKS — Member: 즐겨찾기 + 차단 목록

**Category:** COMPONENT  
**Implementation Status:** IMPLEMENT  
**Seq:** 48  
**Task ID:** COMP-SCR005-MY-ACTIVITY-FAVORITES-BLOCKS

---

## Context

SCR-005(`/account`) 내부에 들어가는 'Member: 즐겨찾기 + 차단 목록' 컴포넌트를 구현하는 Task다. 이 컴포넌트는 해당 Screen의 Page Owner Task가 조립할 때 사용되며, 독립적으로 라우팅되지 않는다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT**로 분류됨(대상 Requirement: REQ-FUNC-040, REQ-FUNC-068). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.
...
- `src/components/scr005/FavoritesBlocksPanel.tsx`(신규)

**이 목록 밖의 파일은 생성·수정하지 않는다.** 다른 파일 변경이 필요하다고 판단되면 이 Task를 완료 처리하지 말고 범위를 재확인한다.

## Functional AC

- 즐겨찾기 0건 시 완성형 Empty State(홈 이동 CTA), 차단 목록+해제 버튼
...
- [Functional] 즐겨찾기 0건 시 완성형 Empty State(홈 이동 CTA)가 의도대로 동작하는지 확인한다.
- [Functional] 차단 목록+해제 버튼가 의도대로 동작하는지 확인한다.
- [Visual] 좌우 2컬럼 배치 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
- [Security/Privacy] 차단 목록은 본인만 조회 조건이 위반되지 않는지 확인한다.
