# TASK-COMP-SCR004-APPLY — 참가 요청 폼
...
SCR-004(`/mates`) 내부에 들어가는 '참가 요청 폼' 컴포넌트를 구현하는 Task다. 이 컴포넌트는 해당 Screen의 Page Owner Task가 조립할 때 사용되며, 독립적으로 라우팅되지 않는다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT**로 분류됨(대상 Requirement: REQ-FUNC-034, REQ-FUNC-035, REQ-FUNC-043). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.

## Requirement Ref

REQ-FUNC-034, REQ-FUNC-035, REQ-FUNC-043
...
- `src/components/scr004/ApplyForm.tsx`(신규)

**이 목록 밖의 파일은 생성·수정하지 않는다.** 다른 파일 변경이 필요하다고 판단되면 이 Task를 완료 처리하지 말고 범위를 재확인한다.

## Functional AC

- 500자 이내 메시지, 중복 PENDING/ACCEPTED 차단(오류 안내), 제출 완료 Toast
...
- [Functional] 500자 이내 메시지가 의도대로 동작하는지 확인한다.
- [Functional] 중복 PENDING/ACCEPTED 차단(오류 안내)가 의도대로 동작하는지 확인한다.
- [Functional] 제출 완료 Toast가 의도대로 동작하는지 확인한다.
- [Visual] 폼은 상세 패널 내 인라인 배치 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
- [Security/Privacy] 메시지는 작성자·요청자만 열람 조건이 위반되지 않는지 확인한다.
