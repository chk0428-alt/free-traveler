# TASK-API-ACCOUNT-PROFILE — 프로필/성인확인/가입 Server Action

**Category:** API  
**Implementation Status:** IMPLEMENT  
**Seq:** 20  
**Task ID:** API-ACCOUNT-PROFILE

---

## Context

'프로필/성인확인/가입 Server Action'를 처리하는 Server Action/Route Handler를 구현하는 Task다. Supabase `user_profile` 테이블에 대한 쓰기/상태 전이를 담당하며, DB-ACCESS 계층을 통해서만 데이터에 접근한다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT**로 분류됨(대상 Requirement: REQ-FUNC-028, REQ-FUNC-029, REQ-FUNC-066). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.

## Requirement Ref

REQ-FUNC-028, REQ-FUNC-029, REQ-FUNC-066

## Screen / Route / Page Entry

- Screen: SCR-005
- Route: `/account`
- Page Entry: `src/app/account/actions/profile.ts`
...
- `src/app/account/actions/profile.ts`(신규)

**이 목록 밖의 파일은 생성·수정하지 않는다.** 다른 파일 변경이 필요하다고 판단되면 이 Task를 완료 처리하지 말고 범위를 재확인한다.

## Functional AC

- 이메일 가입/인증/로그인/로그아웃/재설정, 닉네임 유일성, 성인확인 시각만 기록
...
- 생년월일 미저장, 비밀번호는 Supabase Auth 위임(자체 저장 금지)

## Test Cases

- [Functional] 이메일 가입/인증/로그인/로그아웃/재설정가 의도대로 동작하는지 확인한다.
- [Functional] 닉네임 유일성가 의도대로 동작하는지 확인한다.
- [Functional] 성인확인 시각만 기록가 의도대로 동작하는지 확인한다.
- [Visual] 해당 없음(N/A) 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
- [Security/Privacy] 생년월일 미저장, 비밀번호는 Supabase Auth 위임(자체 저장 금지) 조건이 위반되지 않는지 확인한다.
