# TASK-COMP-SCR004-LIST — 동행글 Card 목록(최대 8개)
...
SCR-004(`/mates`) 내부에 들어가는 '동행글 Card 목록(최대 8개)' 컴포넌트를 구현하는 Task다. 이 컴포넌트는 해당 Screen의 Page Owner Task가 조립할 때 사용되며, 독립적으로 라우팅되지 않는다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT**로 분류됨(대상 Requirement: REQ-FUNC-033, REQ-FUNC-037). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.
...
- 데이터 있으면 최대 8개 우선 노출, 모집중/마감(조회 시점 계산) 배지, 연락처 비노출; 결과 0건 시 완성형 Empty State(조건 초기화+작성 CTA+이용 방법)

## Visual AC

- Desktop 3~4열/Mobile 1열; 데이터 로드 전 목록 스켈레톤(Loading) 상태를 표시한다(`design-reference/UI_CONTRACT.md` 4절 상태 정의)

## Security/Privacy AC

- HTML 응답에 이메일/전화번호 미포함

## Test Cases

- [Functional] 데이터 있으면 최대 8개 우선 노출가 의도대로 동작하는지 확인한다.
- [Functional] 모집중/마감(조회 시점 계산)가 의도대로 동작하는지 확인한다.
- [Functional] 연락처 비노출가 의도대로 동작하는지 확인한다.
- [Functional] 결과 0건 시 완성형 Empty State(조건 초기화+작성 CTA+이용 방법)가 의도대로 동작하는지 확인한다.
- [Visual] Desktop 3~4열/Mobile 1열 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
- [Visual] 데이터 로드 전 목록 스켈레톤(Loading) 상태가 표시되는지 확인한다.
- [Security/Privacy] HTML 응답에 이메일/전화번호 미포함 조건이 위반되지 않는지 확인한다.
