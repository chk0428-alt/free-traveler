# TASK-INFRA-DESIGN-TOKENS — 디자인 토큰/Tailwind 설정

**Category:** INFRA  
**Implementation Status:** IMPLEMENT  
**Seq:** 1  
**Task ID:** INFRA-DESIGN-TOKENS

---

## Context

Screen에 종속되지 않는 공통 인프라/유틸리티 Task '디자인 토큰/Tailwind 설정'다. 여러 Screen의 Component/Page Owner Task가 이 결과물에 의존한다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT**로 분류됨(대상 Requirement: REQ-NF-006, REQ-NF-023). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.

## Requirement Ref

REQ-NF-006, REQ-NF-023
...
- D-001 색상(coral #F2613B 등)·타이포(Inter+시스템 한글)·spacing(64~96/40~64)·radius·shadow 토큰이 코드 상수로 정의된다

## Visual AC

- 토큰 값이 D-001 표와 1:1 일치(임의 색상 추가 없음)
...
- [Functional] D-001 색상(coral #F2613B 등)·타이포(Inter+시스템 한글)·spacing(64~96/40~64)·radius·shadow 토큰이 코드 상수로 정의된다가 의도대로 동작하는지 확인한다.
- [Visual] 토큰 값이 D-001 표와 1:1 일치(임의 색상 추가 없음) 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
