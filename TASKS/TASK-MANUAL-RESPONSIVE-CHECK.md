# TASK-MANUAL-RESPONSIVE-CHECK — 반응형/터치영역 브라우저 수동 확인
...
자동화 도구로 완전히 대체할 수 없어 브라우저에서 사람이 직접 확인해야 하는 '반응형/터치영역 브라우저 수동 확인' Task다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT(축소)**로 분류됨(대상 Requirement: REQ-FUNC-065). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.
...
- 320px/390px/768px/1440px 뷰포트에서 가로 스크롤·요소 겹침 없음, 인터랙션 요소 44×44px 이상

## Visual AC

- 5개 Screen 모두 Section 상하 여백·Card 열 수가 D-001 15절 범위 준수
...
- [Functional] 320px/390px/768px/1440px 뷰포트에서 가로 스크롤·요소 겹침 없음가 의도대로 동작하는지 확인한다.
- [Functional] 인터랙션 요소 44×44px 이상가 의도대로 동작하는지 확인한다.
- [Visual] 5개 Screen 모두 Section 상하 여백·Card 열 수가 D-001 15절 범위 준수 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.

## Verify

- 브라우저 수동 확인(DevTools 뷰포트 전환)
