# TASK-UNIT-TRAVEL-DATES — 날짜 검증 단위 테스트
...
'날짜 검증 단위 테스트'에 대한 단위 테스트 Task다. 관련 구현 Task의 순수 로직(날짜 계산, 패턴 탐지, 상태 전이)을 코드 레벨에서 검증한다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT**로 분류됨(대상 Requirement: REQ-FUNC-013, REQ-FUNC-021). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.
...
- `src/lib/__tests__/travel-dates.test.ts`(신규)

**이 목록 밖의 파일은 생성·수정하지 않는다.** 다른 파일 변경이 필요하다고 판단되면 이 Task를 완료 처리하지 말고 범위를 재확인한다.

## Functional AC

- 과거 출발일/체크인, 귀국일<출발일, 체크아웃≤체크인 등 경계값 케이스 전부 차단 검증
...
- [Functional] 과거 출발일/체크인가 의도대로 동작하는지 확인한다.
- [Functional] 귀국일<출발일가 의도대로 동작하는지 확인한다.
- [Functional] 체크아웃≤체크인 등 경계값 케이스 전부 차단 검증가 의도대로 동작하는지 확인한다.
