# TASK-COMP-SCR003-HOTEL-FORM — 숙소 찾기 탭(폼·검증·요약·외부 이동)
...
SCR-003(`/travel-tools`) 내부에 들어가는 '숙소 찾기 탭(폼·검증·요약·외부 이동)' 컴포넌트를 구현하는 Task다. 이 컴포넌트는 해당 Screen의 Page Owner Task가 조립할 때 사용되며, 독립적으로 라우팅되지 않는다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT**로 분류됨(대상 Requirement: REQ-FUNC-019, REQ-FUNC-020, REQ-FUNC-021, REQ-FUNC-022, REQ-FUNC-023, REQ-FUNC-024, REQ-FUNC-025, REQ-FUNC-026). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.

## Requirement Ref

REQ-FUNC-019, REQ-FUNC-020, REQ-FUNC-021, REQ-FUNC-022, REQ-FUNC-023, REQ-FUNC-024, REQ-FUNC-025, REQ-FUNC-026
...
- 국가/지역/체크인/체크아웃 필수, 체크아웃≤체크인 차단, 요약+비전달 고지, 외부 이동 새 탭, 서버 미전송

## Visual AC

- FlightForm과 동일 패턴(재사용 가능한 Form 스펙); 외부 이동 버튼 클릭 직후 버튼 비활성화+로딩 표시(Loading 상태, `design-reference/UI_CONTRACT.md` 3절)
...
- [Functional] 국가/지역/체크인/체크아웃 필수가 의도대로 동작하는지 확인한다.
- [Functional] 체크아웃≤체크인 차단가 의도대로 동작하는지 확인한다.
- [Functional] 요약+비전달 고지가 의도대로 동작하는지 확인한다.
- [Functional] 외부 이동 새 탭가 의도대로 동작하는지 확인한다.
- [Functional] 서버 미전송가 의도대로 동작하는지 확인한다.
- [Visual] FlightForm과 동일 패턴(재사용 가능한 Form 스펙) 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
