# TASK-COMP-SCR005-MY-ACTIVITY-POSTS — Member: 내 모집글/참가 요청 목록

**Category:** COMPONENT  
**Implementation Status:** IMPLEMENT  
**Seq:** 47  
**Task ID:** COMP-SCR005-MY-ACTIVITY-POSTS

---

## Context

SCR-005(`/account`) 내부에 들어가는 'Member: 내 모집글/참가 요청 목록' 컴포넌트를 구현하는 Task다. 이 컴포넌트는 해당 Screen의 Page Owner Task가 조립할 때 사용되며, 독립적으로 라우팅되지 않는다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT**로 분류됨(대상 Requirement: REQ-FUNC-036, REQ-FUNC-038, REQ-FUNC-043). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.

## Requirement Ref

REQ-FUNC-036, REQ-FUNC-038, REQ-FUNC-043
...
- `src/components/scr005/MyPostsPanel.tsx`(신규)

**이 목록 밖의 파일은 생성·수정하지 않는다.** 다른 파일 변경이 필요하다고 판단되면 이 Task를 완료 처리하지 말고 범위를 재확인한다.

## Functional AC

- 내 글(상태·받은요청수), 내 요청(PENDING/ACCEPTED/REJECTED) 목록, 데이터 없으면 완성형 Empty State

## Visual AC

- Card 목록형, 배지로 상태 구분(텍스트 병기)
...
- [Functional] 내 글(상태·받은요청수)가 의도대로 동작하는지 확인한다.
- [Functional] 내 요청(PENDING/ACCEPTED/REJECTED)가 의도대로 동작하는지 확인한다.
- [Functional] 데이터 없으면 완성형 Empty State가 의도대로 동작하는지 확인한다.
- [Visual] Card 목록형, 배지로 상태 구분(텍스트 병기) 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
- [Security/Privacy] 본인 데이터만 조회(RLS) 조건이 위반되지 않는지 확인한다.
