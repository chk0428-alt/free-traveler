# TASK-INFRA-PLAYWRIGHT-CONFIG — Playwright Chromium 프로젝트 설정

**Category:** INFRA  
**Implementation Status:** IMPLEMENT  
**Seq:** 8  
**Task ID:** INFRA-PLAYWRIGHT-CONFIG

---

## Context

Screen에 종속되지 않는 공통 인프라/유틸리티 Task 'Playwright Chromium 프로젝트 설정'다. 여러 Screen의 Component/Page Owner Task가 이 결과물에 의존한다.
...
- `package.json`(devDependency 추가)

**이 목록 밖의 파일은 생성·수정하지 않는다.** 다른 파일 변경이 필요하다고 판단되면 이 Task를 완료 처리하지 말고 범위를 재확인한다.

## Functional AC

- `projects` 배열에 Chromium 1개만 정의(Firefox/WebKit 없음)
...
- [Functional] `projects` 배열에 Chromium 1개만 정의(Firefox/WebKit 없음)가 의도대로 동작하는지 확인한다.
