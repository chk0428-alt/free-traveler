# TASK-API-MATE-REPORT — 신고 접수/상태 변경 Server Action

**Category:** API  
**Implementation Status:** IMPLEMENT  
**Seq:** 18  
**Task ID:** API-MATE-REPORT

---

## Context

'신고 접수/상태 변경 Server Action'를 처리하는 Server Action/Route Handler를 구현하는 Task다. Supabase `report` 테이블에 대한 쓰기/상태 전이를 담당하며, DB-ACCESS 계층을 통해서만 데이터에 접근한다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT**로 분류됨(대상 Requirement: REQ-FUNC-039, REQ-FUNC-041). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.

## Requirement Ref

REQ-FUNC-039, REQ-FUNC-041

## Screen / Route / Page Entry

- Screen: SCR-004, SCR-005
- Route: `/mates`, `/account`
- Page Entry: `src/app/mates/actions/report.ts`
...
- `src/app/mates/actions/report.ts`(신규)

**이 목록 밖의 파일은 생성·수정하지 않는다.** 다른 파일 변경이 필요하다고 판단되면 이 Task를 완료 처리하지 말고 범위를 재확인한다.

## Functional AC

- 신고 접수 시 접수 ID 즉시 반환, Admin만 OPEN/REVIEWING/RESOLVED/DISMISSED 상태 변경 가능

## Visual AC

- 접수 확인 Toast(3초 이내 노출)

## Security/Privacy AC

- 신고자/피신고자 상세는 Admin만 열람(RLS)

## Test Cases

- [Functional] 신고 접수 시 접수 ID 즉시 반환가 의도대로 동작하는지 확인한다.
- [Functional] Admin만 OPEN/REVIEWING/RESOLVED/DISMISSED 상태 변경 가능가 의도대로 동작하는지 확인한다.
- [Visual] 접수 확인 Toast(3초 이내 노출) 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
- [Security/Privacy] 신고자/피신고자 상세는 Admin만 열람(RLS) 조건이 위반되지 않는지 확인한다.
