# TASK-COMP-SCR001-CARDGRID — 국내/해외 여행지 Card Grid

**Category:** COMPONENT  
**Implementation Status:** IMPLEMENT  
**Seq:** 23  
**Task ID:** COMP-SCR001-CARDGRID

---

## Context

SCR-001(`/`) 내부에 들어가는 '국내/해외 여행지 Card Grid' 컴포넌트를 구현하는 Task다. 이 컴포넌트는 해당 Screen의 Page Owner Task가 조립할 때 사용되며, 독립적으로 라우팅되지 않는다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT**로 분류됨(대상 Requirement: REQ-FUNC-004, REQ-FUNC-007). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.
...
- 국내 6개·해외 6개 Card, 각 Card에 alt 있는 이미지·테마 태그, 클릭 시 상세 Drawer 오픈(라우트 이동 아님)

## Visual AC

- Desktop 3~4열/Mobile 1열, Card 간격 24px/16px; 데이터 로드 전에는 Card 스켈레톤(Loading) 상태를 표시한다(`design-reference/UI_CONTRACT.md` 1절 상태 정의)
...
- [Functional] 국내 6개·해외 6개 Card가 의도대로 동작하는지 확인한다.
- [Functional] 각 Card에 alt 있는 이미지·테마 태그가 의도대로 동작하는지 확인한다.
- [Functional] 클릭 시 상세 Drawer 오픈(라우트 이동 아님)가 의도대로 동작하는지 확인한다.
- [Visual] Desktop 3~4열/Mobile 1열, Card 간격 24px/16px 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
- [Visual] 데이터 로드 전 Card 스켈레톤(Loading) 상태가 표시되는지 확인한다.
