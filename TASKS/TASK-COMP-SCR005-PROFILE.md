# TASK-COMP-SCR005-PROFILE — Member: 프로필 요약 + 수정
...
SCR-005(`/account`) 내부에 들어가는 'Member: 프로필 요약 + 수정' 컴포넌트를 구현하는 Task다. 이 컴포넌트는 해당 Screen의 Page Owner Task가 조립할 때 사용되며, 독립적으로 라우팅되지 않는다.

## Project Scope

`docs/PROJECT_SCOPE.md`에서 **IMPLEMENT**로 분류됨(대상 Requirement: REQ-FUNC-029). EXCLUDED 항목이 아니므로 상세 구현 Task로 존재한다.
...
- `src/components/scr005/ProfilePanel.tsx`(신규)

**이 목록 밖의 파일은 생성·수정하지 않는다.** 다른 파일 변경이 필요하다고 판단되면 이 Task를 완료 처리하지 말고 범위를 재확인한다.

## Functional AC

- 닉네임/연령대/여행스타일 필수, 성별 선택, 성인확인 완료 배지

## Visual AC

- 좌우 분할(정보+수정 버튼); 탭 전환·목록 로드 시 스켈레톤(Loading) 상태를 표시한다(`design-reference/UI_CONTRACT.md` 5절 상태 정의)
...
- [Functional] 닉네임/연령대/여행스타일 필수가 의도대로 동작하는지 확인한다.
- [Functional] 성인확인 완료 배지가 의도대로 동작하는지 확인한다.
- [Visual] 좌우 분할(정보+수정 버튼) 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
- [Visual] 탭 전환·목록 로드 시 스켈레톤(Loading) 상태가 표시되는지 확인한다.
