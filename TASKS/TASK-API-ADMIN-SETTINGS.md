# TASK-API-ADMIN-SETTINGS — 외부 URL 설정 Server Action

**Category:** API  
**Implementation Status:** IMPLEMENT  
**Seq:** 21  
**Task ID:** API-ADMIN-SETTINGS

---

## Context

'외부 URL 설정 Server Action'를 처리하는 Server Action/Route Handler를 구현하는 Task다. Supabase `outbound_link_setting, user_profile(권한 확인용 읽기)` 테이블에 대한 쓰기/상태 전이를 담당하며, DB-ACCESS 계층을 통해서만 데이터에 접근한다.
...
- Screen: SCR-005
- Route: `/account`
- Page Entry: `src/app/account/actions/admin-settings.ts`
...
- `src/app/account/actions/admin-settings.ts`(신규)

**이 목록 밖의 파일은 생성·수정하지 않는다.** 다른 파일 변경이 필요하다고 판단되면 이 Task를 완료 처리하지 말고 범위를 재확인한다.

## Functional AC

- Admin만 항공/숙소 URL 저장, HTTPS·허용목록 검증, 실패 시 인라인 오류
...
- `javascript:`/`http:` URL 저장 차단, 비Admin 요청 403

## Test Cases

- [Functional] Admin만 항공/숙소 URL 저장가 의도대로 동작하는지 확인한다.
- [Functional] HTTPS·허용목록 검증가 의도대로 동작하는지 확인한다.
- [Functional] 실패 시 인라인 오류가 의도대로 동작하는지 확인한다.
- [Visual] 저장 완료 Toast 조건을 Desktop 1440px/Mobile 390px에서 육안 확인한다.
- [Security/Privacy] `javascript:`/`http:` URL 저장 차단, 비Admin 요청 403 조건이 위반되지 않는지 확인한다.
