# TASK-COMP-SCR004-INTRO — Intro + 글 작성 CTA

**Category:** COMPONENT  
**Implementation Status:** IMPLEMENT  
**Seq:** 39  
**Task ID:** COMP-SCR004-INTRO

---

## Context

SCR-004(`/mates`) 내부에 들어가는 'Intro + 글 작성 CTA' 컴포넌트를 구현하는 Task다. 이 컴포넌트는 해당 Screen의 Page Owner Task가 조립할 때 사용되며, 독립적으로 라우팅되지 않는다.
...
- `src/components/scr004/Intro.tsx`(신규)

**이 목록 밖의 파일은 생성·수정하지 않는다.** 다른 파일 변경이 필요하다고 판단되면 이 Task를 완료 처리하지 말고 범위를 재확인한다.

## Functional AC

- 성인 인증 회원만 작성 가능 안내, `/travel-tools` 동행 탭 CTA
...
- [Functional] 성인 인증 회원만 작성 가능 안내가 의도대로 동작하는지 확인한다.
- [Functional] `/travel-tools` 동행 탭 CTA가 의도대로 동작하는지 확인한다.
