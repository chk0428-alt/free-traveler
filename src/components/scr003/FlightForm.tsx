...
import { validateFlightDates } from "@/lib/travel-dates";
...
export type FlightFormProps = {
  /** 관리자가 설정한 항공 검색 URL(SCR-005 관리자 탭). Page Owner가 서버에서 조회해 전달한다. */
  flightUrl?: string | null;
};

export function FlightForm({ flightUrl }: FlightFormProps) {
  ...
  const summary = useMemo(
    () =>
      showSummary
        ? `${country} ${region} · ${departureDate} ~ ${returnDate}`
        : null,
    [showSummary, country, region, departureDate, returnDate],
  );
  ...
  function handleCheckSummary() {
    if (!country || !region) {
      setError("국가·지역·출발일·귀국일을 모두 입력해 주세요.");
      setShowSummary(false);
      return;
    }
    const dateError = validateFlightDates(departureDate, returnDate);
    if (dateError === "MISSING") {
      setError("국가·지역·출발일·귀국일을 모두 입력해 주세요.");
      setShowSummary(false);
      return;
    }
    if (dateError === "PAST_DEPARTURE") {
      setError("출발일은 오늘 이후여야 합니다.");
      setShowSummary(false);
      return;
    }
    if (dateError === "RETURN_BEFORE_DEPARTURE") {
      setError("귀국일은 출발일보다 빠를 수 없습니다.");
      setShowSummary(false);
      return;
    }
    setError(null);
  ...
  function handleOpenFlightSite() {
    setIsNavigating(true);
    setLinkError(null);
    const result = openOutboundLink(flightUrl);
    if (!result.ok) {
      ...
      {summary ? (
        <div className="flex flex-col gap-sm rounded-sm bg-surface-soft p-lg">
          ...
          <p className="text-body-sm text-muted">
            입력값은 외부 사이트로 전달되지 않습니다. 실제 항공편 가격·재고는
            이동한 사이트에서 확인해 주세요.
          </p>
          {linkError ? (
          ...
          <a
            href={flightUrl ?? undefined}
            target="_blank"
            rel="noopener noreferrer"
            onClick={(event) => {
              event.preventDefault();
              handleOpenFlightSite();
            }}
